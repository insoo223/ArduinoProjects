...
//---------------------------------------------
// added by Insoo (Thu, May 23, 2019)
void RTC_DS1307::setupDS1307 () 
{
  while (!Serial)
  ...
  if (!isrunning()) 
  {
    ...
    //rtc.adjust(DateTime(2019, 4, 16, 9, 50, 0));
  }
}//setupDS1307

//---------------------------------------------
// added by Insoo (Thu, May 23, 2019)
void RTC_DS1307::dispTimeDS1307_Serial() 
{
    ...
    Serial.print(" (");
    Serial.print(_daysOfTheWeek[currentTime.dayOfTheWeek()]);
    Serial.print(") ");
    ...
    Serial.println();
}//dispTimeDS1307_Serial

//---------------------------------------------
// added by Insoo (Thu, May 23, 2019)
void RTC_DS1307::dispYYYYMMDD_Mdigit7Seg(trippleX* X, Four7segX* _7X) 
{
    DateTime currentTime = now();
    
    _7X->dispMdigits(X, currentTime.year(), 0, 50, DISP_NUM_NORMAL);
    _7X->dispMdigits(X, currentTime.month()*100 + currentTime.day(),0,50, DISP_NUM_NORMAL);
    _7X->dispMdigits(X, currentTime.dayOfTheWeek(),0,50, DISP_NUM_NORMAL);
}//dispYYYYMMDD_Mdigit7Seg

//---------------------------------------------
// added by Insoo (Thu, May 23, 2019)
void RTC_DS1307::dispHHMM_Mdigit7Seg(trippleX* X, Four7segX* _7X)
{
    DateTime currentTime = now();
    
    _7X->dispMdigits(X, currentTime.hour()*100 + currentTime.minute(),0,500, DISP_NUM_NORMAL);
}//dispTimeHHMM_DS1307_4digit7Seg

//---------------------------------------------
// added by Insoo (Thu, May 23, 2019)
void RTC_DS1307::setTimeDS1307(int year, byte month, byte dayOfMonth, byte hour, byte minute, byte second)
{
    // This line sets the RTC with an explicit date & time, for example to set
    // January 21, 2014 at 3am you would call:
    // rtc.adjust(DateTime(2014, 1, 21, 3, 0, 0));
    adjust( DateTime(year, month, dayOfMonth, hour, minute, second) );
}//setTimeDS1307
